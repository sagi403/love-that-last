...
it("responds with 401 if not authenticated", async () => {
  return request(app).get(`/api/orders/myorders`).expect(401);
});
...
it("returns a 400 for not providing a validated query", async () => {
  ...
  await request(app)
    .get("/api/orders/myorders?pageNumber=notValid")
    .set("Cookie", cookie)
    .expect(400);
});
...
it("responds with 200 for getting user orders without providing a query", async () => {
  const cookie = await global.getCookie();
...
it("responds with 200 for successfully getting user orders with a query provided", async () => {
  const cookie = await global.getCookie();
  ...
  const response = await request(app)
    .get(`/api/orders/myorders?pageNumber=1`)
    .set("Cookie", cookie)
    .expect(200);
...
it("responds with 200 for successfully getting user orders from the second page", async () => {
  const cookie = await global.getCookie();
  ...
  const response = await request(app)
    .get(`/api/orders/myorders?pageNumber=2`)
    .set("Cookie", cookie)
    .expect(200);
