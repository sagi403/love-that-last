...
it("response with 401 if not authenticated", async () => {
  ...
  return request(app).get(`/api/users/${id}`).expect(401);
});
...
it("responds with 401 if user is not an admin", async () => {
  ...
  return request(app).get(`/api/users/${id}`).set("Cookie", cookie).expect(401);
});
...
it("returns a 404 if user was not found", async () => {
  ...
  return request(app).get(`/api/users/${id}`).set("Cookie", cookie).expect(404);
});

it("returns a 200 for finding a valid user", async () => {
  const cookie = await global.getCookie();
